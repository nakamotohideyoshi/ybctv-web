...
add_action( 'admin_menu', 'homepage_content_menu' );

function homepage_content_menu() {
  add_options_page( 
    'Home Page Content',
    'Home Page Content',
    'manage_options',
    'homepage-content.php',
    'homepage_content_settings_page'
  );
}

function register_homepage_content_options() {
  //register our settings
  register_setting( 'homepage-content-settings-group', 'toggle_headtohead' );
  register_setting( 'homepage-content-settings-group', 'toggle_portfolios' );
  register_setting( 'homepage-content-settings-group', 'toggle_variable' );
  register_setting( 'homepage-content-settings-group', 'variable_category' );
}

add_action( 'admin_init', 'register_homepage_content_options' );

function homepage_content_settings_page() {
?>
<div class="wrap">
<h1>Home Page Content</h1>

<form method="post" action="options.php">
    <?php settings_fields( 'homepage-content-settings-group' ); ?>
    <?php do_settings_sections( 'homepage-content-settings-group' ); ?>
    <table class="form-table">
        <tr valign="top">
        <th scope="row">Toggle Head to Head Section</th>
        <td><input name="toggle_headtohead" type="checkbox" value="1" <?php checked( '1', get_option( 'toggle_headtohead' ) ); ?> /></td>
        </tr>
         
        <tr valign="top">
        <th scope="row">Toggle Portfolios Section</th>
        <td><input name="toggle_portfolios" type="checkbox" value="1" <?php checked( '1', get_option( 'toggle_portfolios' ) ); ?> /></td>
        </tr>
        
        <tr valign="top">
        <th scope="row">Toggle Variable Section</th>
        <td><input name="toggle_variable" type="checkbox" value="1" <?php checked( '1', get_option( 'toggle_variable' ) ); ?> /></td>
        </tr>
        
        <tr valign="top">
        <th scope="row">Select Variable Section Category</th>
        <?php 
        $categories = get_categories('hide_empty=0&orderby=name');
        ?>
        <td>
        <select name="variable_category">
          <?php foreach ($categories as $cat ) { 
            ?>
            <option value="<?php echo $cat->term_id; ?>" <?php selected(get_option('variable_category'), $cat->term_id); ?>><?php echo $cat->cat_name; ?></option>
          <?php } ?>
        </select>
        </td>
        </tr>
