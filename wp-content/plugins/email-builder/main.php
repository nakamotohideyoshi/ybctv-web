						...
						if ( isset($static[$staticKey]->Version) && isset($static[$staticKey]->Data) && $static[$staticKey]->Version == 2 ) 
						{
							...
							if ( is_array($data) ) 
							{
								foreach ( $data as $key => $value ) 
								{
                                    ...
                                    if ( $key == 'subtitle' && strpos($staticEntity->Type, 'Sponsored_Content_2') !== false )
									{
                                    ...
                                    if ( $key == 'excerpt' && strpos($staticEntity->Type, 'Sponsored_Content_2') !== false )
									{
										$value = str_replace( '<a ', '<a style="font-weight: normal; text-decoration: none; color: ' . $colors[ $static[$staticKey]->Site ] . '" ', $value );
									}
...
WHERE EmailID=" . $lastid);
    ...
    register_rest_route( 
        ...
        array(
            ...
            'callback' => function ($params ){
	            ...
	            $latest_portfolio= $wpdb->get_results("select * from wp_2_posts where post_type='post' and post_status='publish' order by post_date desc, ID desc limit 1");
	            $latest_international= $wpdb->get_results("select * from wp_3_posts where post_type='post' and post_status='publish' order by  post_date desc, ID desc limit 1");
	            $latest_fundselector= $wpdb->get_results("select * from wp_4_posts where post_type='post' and post_status='publish' order by  post_date desc, ID desc limit 1");
	            $latest_expertinvestor= $wpdb->get_results("select * from wp_5_posts where post_type='post' and post_status='publish' order by  post_date desc, ID desc limit 1");
				if ($wpdb->last_error) {
  					$response = new WP_REST_Response( $wpdb->last_error );
					return $response;
				}
	...
	public function load_bundle() {
		...
		$path = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR;
		$createdAtMax = 0;
		$files = scandir($path);
		$scriptFile = "";
		
		foreach ( $files as $file ) {
			if ( $file != "." && $file != ".." && strpos($file, ".js.map") == false ) {
				$createdAt = filemtime( $path . $file );
		...
		wp_enqueue_script( 'plugin-scripts', plugins_url('js/' . $scriptFile, __FILE__),array(),  '0.0.1', true );
	}
